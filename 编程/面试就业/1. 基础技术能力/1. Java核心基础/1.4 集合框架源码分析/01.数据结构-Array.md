...
> 
>
> 只有**挨着存放**和**分开存放**。所以，数据结构的核心，就是**数组**和**链表**。
...
- [x] 请说明 `ArrayList` 在 Java 集合框架中的位置及作用。
- [x] `ArrayList` 能存储不同类型的元素吗？为什么？
- [x] 对比 `ArrayList` 和普通数组，它们的优缺点分别是什么？

- [x] 为什么说 `ArrayList` 是非线程安全的，举例说明可能出现的问题。
- [x] 除了 `Vector`、`Collections.synchronizedList` 和 `CopyOnWriteArrayList`，还有其他保证 `ArrayList` 线程安全的方式吗？
- [x] 在高并发场景下，`CopyOnWriteArrayList` 替代 `ArrayList` 有哪些优势和不足？

- [x] `ArrayList` 初始化时如果不指定容量，默认容量是多少，这个默认值是如何确定的？
- [x] 当向 `ArrayList` 中频繁添加元素时，扩容操作会带来哪些性能问题？Arrays.copyOf()
- [x] 请描述 `ArrayList` 中删除元素时，底层数组是如何处理的。

- [x] 如何在 `ArrayList` 的指定位置插入元素，插入操作的时间复杂度是多少？
- [x] 若要对 `ArrayList` 中的元素进行排序，有哪些方法可以实现？
- [x] 怎样高效地删除 `ArrayList` 中**所有满足特定条件**的元素？迭代器最佳方案。

- [x] 对比 `ArrayList` 和 `LinkedList`，在不同操作（如随机访问、插入删除）下的性能表现如何？
...
> 从前往后删, js和java都有这个效果。跳着删。

![image-20250416094655283](https://woniumd.oss-cn-hangzhou.aliyuncs.com/security/mayuhang/202504160946692.png)
 ...
 * @Author: 马宇航
 * @Todo: 快速失败
 * @DateTime: 25/04/16/星期三 09:41
 * @Component: 成都蜗牛学苑
 **/
public class FastFailArrayList {
    public static void main(String[] args) {
...
Itr私有内部类的属性：new Itr()出来后，expectedModCount 就有值

```java
int cursor;       // 要返回的下一个元素的索引， 游标，把他当做指针 jdbc操作数据库
int lastRet = -1; // 返回的最后一个元素的索引;如果没有这样的元素就是 -1
int expectedModCount = modCount; //预期 操作 数量 modCounts是来自于ArrayList
```
...
> 
>
> 我们只需要保证，modCount和expectedModCount不一样，就会触发并发修改异常，也就是快速失败。
>
> 也就是说，在itr.next()操作的时候，如下列代码：
>
> ```java
>     public E next() {
>         //一来就检查 并发修改问题
...
>出现问题的原因：
>
>1. 迭代器操作的时候，一定都只要用迭代操作，别去做其他的原始ArrayList的操作。
>
>2. 如果想要迭代新增，修改等，用ListItr 私有内部类实现就ok。
>3. 并发更会出现 快速失败问题。
>
>![image-20250416101216366](https://woniumd.oss-cn-hangzhou.aliyuncs.com/security/mayuhang/202504161012673.png)

### 4. list.toString的快速失败问题
...
/**
 * @Author: 马宇航
 * @Todo: 并发中出现的问题  1.7版本里面,会在并发的时候出现死循环。
 * @DateTime: 25/04/16/星期三 10:32
 * @Component: 成都蜗牛学苑
 **/
public class ConcurrentArrayListProblem {
    public static void main(String[] args) {
        List list = new ArrayList();
        for (int i = 0; i < 10; i++) {
            list.add(i);
...
JUC（java.util.concurrent包 ）环节，来讲价CopyOnWriteArrayList的底层源码。里面会涵盖另外一个安全失败问题 fail-safe。
